...
onMounted(async (): Promise<void> => {
  try {
    ...
    const result: AxiosResponse<FetchCustomer> = await api.get(
      `customer/${route.params.customerId}`,
    )
    const customer: Customer = result.data.data as Customer
    form.code = customer.kode
...
const send = async (): Promise<void> => {
  try {
    isLoading.value = true
    const result: AxiosResponse<FetchCustomer> = await api.put(
      `customer/${route.params.customerId}`,
      {
        name: form.name,
        kode: form.code,
